...
class Slack(object):
	...
	def channels_invite(self, channel_name):
		...
		for i in invites.split(" "):
			...
			url = "https://slack.com/api/channels.invite?token={token}&channel={channel_id}&user={user}".format(token=settings.token,
																											 channel_id=channel_id,
																											 user=user_id)
			response = requests.get(url).json()
			if response["ok"]:
	...
	def channels_create(self):
		...
		url = "https://slack.com/api/channels.create?token={token}&name={name}&purpose={purpose}".format(token=settings.token,
																										 name=name,
																										 purpose=purpose)
		response = requests.get(url).json()
